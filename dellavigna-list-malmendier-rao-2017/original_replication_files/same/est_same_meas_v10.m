% =================================================================
% Estimate the model
% Same parameters for V/NV, 10% voters mismeasured
% =================================================================
...
dataFile = strcat('same-meas-v10-',datestr(now,'dd-mmm-yyyy-HH_MM'));
outputFile = strcat(dataFile,'.txt');
diary(strcat(outputDir,outputFile));

% Use measurement error 10% of apparent voters are non-voters
...
paramsToUse=fixedRho;  % OR: fixedL_Rho fixedRho
display 'fixedRho';
...
rng(747)
[searchInits,lower_start,upper_start] = getSearchInits(noSearchInits,L_setting,...
...
upperBound=[1,    1,  0.5,  100, 100, 100, 200,  100,  100, 100, 100,  1,  50,  500, 500];
end
if L_setting == 'dist' 
lowerBound=[eps, eps, eps, -100, eps, eps, eps, -100, -100, eps, eps, -1, eps, -500, eps];
upperBound=[ 1,    1, 0.5,  100, 100, 100, 200,  100,  100, 100, 100,  1,  1,   500, 500];
end
