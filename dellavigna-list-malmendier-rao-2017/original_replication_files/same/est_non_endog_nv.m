% =================================================================
% Estimate model for non-voters only
% =================================================================
...
curDir = pwd;
mkdir('../output','non_endog');
outputDir=('../output/non_endog/');

dataFile = strcat('non-endog-nv-',datestr(now,'dd-mmm-yyyy-HH_MM'));
outputFile = strcat(dataFile,'.txt');
diary(strcat(outputDir,outputFile));

% set votesim file that generates moments
votesimFile = @voteSim_vnv_sep
calc_implicationsFile = @calc_implications_vnv_sep
...
fixed_mu_eps = -99999; % or -99999 for non-voters
fixed_sigma_eps = 0;
...
paramsToUse=fixed_LRhoEps; 
display 'fixed_LRhoEps : fix L, rho, mu and sigma eps';
momentsToUse=nonvoterMoments; % or nonvoterMoments
display 'nonvoterMoments'; % or nonvoterMoments
...
rng(747)
[searchInits,lower_start,upper_start] = getSearchInits_vnv_sep(noSearchInits,L_setting,...
...
%Define the search space - depends on L or L_lambda
 if L_setting == 'cons' 
lowerBound=[eps, eps, eps, -100, eps, eps, eps, -100, -100, eps, eps, -1, eps, -999999, 0];
upperBound=[1,    1,  0.5,  100, 100, 100, 200,  100,  100, 100, 100,  1,  50,  999999, 999999];
end
if L_setting == 'dist' 
lowerBound=[eps, eps, eps, -100, eps, eps, eps, -100, -100, eps, eps, -1, eps, -999999, 0];
upperBound=[ 1,    1, 0.5,  100, 100, 100, 200,  100,  100, 100, 100,  1,  1,   999999, 999999];
end
