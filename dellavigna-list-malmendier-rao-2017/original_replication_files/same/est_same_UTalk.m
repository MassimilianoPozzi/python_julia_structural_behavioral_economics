% =================================================================
% Estimate the model
% Same parameters for V/NV, estimate utility of talking about politics
% =================================================================
...
dataFile = strcat('same-Utalk-',datestr(now,'dd-mmm-yyyy-HH_MM'));
...
%Set params and fix values for UTALK:
noOfParams=17;
allParams = 1:noOfParams;
fixedRho=[1:11 13:17]; 
fixedRho_MuSv=[1:7 9:11 13:17];
...
% For UTalk Robustness: mu sv fix to 0, add values for utalk (also 0)
% Sets value to use when parameters are designated as fixed
if L_setting == 'cons' 
svyEstimates  = [0.37, 0.34, 0.1, -28, 30, 1, 42, 0, 0, 15, 15, 0, ...
    cons_fixedL, fixed_mu_eps, fixed_sigma_eps, 0, 0]';
end
if L_setting == 'dist' 
svyEstimates  = [0.37, 0.34, 0.1, -28, 30, 1, 42, 0, 0, 15, 15, 0, ...
    0.05, fixed_mu_eps, fixed_sigma_eps, 0, 0]';
end
...
rng(747)
[searchInits,lower_start,upper_start] = getSearchInits_UTalk(noSearchInits,L_setting,...
    sim_voters,rand_set,votesimFile);
for j=1:noOfParams
...
% Range for utility of talking is -100 to 100
%Define the search space - depends on L or L_lambda
if L_setting == 'cons' 
lowerBound=[eps, eps, eps, -100, eps, eps, eps, -100, -100, eps, eps, -1, eps, -500, eps, -100, -100];
upperBound=[1,    1,  0.5,  100, 100, 100, 200,  100,  100, 100, 100,  1,  50,  500, 500,  100,  100];
end
if L_setting == 'dist' 
lowerBound=[eps, eps, eps, -100, eps, eps, eps, -100, -100, eps, eps, -1, eps, -500, eps, -100, -100];
upperBound=[ 1,    1, 0.5,  100, 100, 100, 200,  100,  100, 100, 100,  1,  1,   500, 500,  100,  100];
end
