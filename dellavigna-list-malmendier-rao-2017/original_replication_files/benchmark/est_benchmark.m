...
dataFile = strcat('benchmark-',datestr(now,'dd-mmm-yyyy-HH-MM'));
outputFile = strcat(dataFile,'.txt');
diary(strcat(outputDir,outputFile));

% set votesim file that generates moments (voteSimEndogenousVoting_vary for
% the benchmark model, other robustness have different files)
% set cdf selection file - this runs vote sim endogenous and then outputs
% extra statistics for the graphs and implications
votesimFile = @voteSimEndogenousVoting_vary
calc_implicationsFile = @calc_implications

% choose whether L is a constant ("cons", fixed L across people) or an 
% exponential distribution ("dist", estimated coefficient is "L_lambda"
% the lambda parameter)
L_setting = 'cons'

% Choose L value if constant and fixed
cons_fixedL = 10;

% Choose mu and sigma epsilon values when fixed
fixed_mu_eps = 50;
fixed_sigma_eps = 200;

% Estimation options:
% Number of simulated potential voters in each simulation
sim_voters=750000
%Number of search start points
noSearchInits=720

% Empirical moments and var-cov matrix: 1 x 100 moments and matrix 100 x 100
% Baseline turnout and GOTV added to empirical moments from Stata
% Baseline turnout: constant coefficient and standard error from Table 6, Col 1
% GOTV turnout: baseline turnout + coeff on the indicator for flyer with
% announcement that household will be asked about voting in the future in
% Table 6, Col 2. The standard error is sqrt(SE^2 on baseline turnout + 
% SE^2 on GOTV effect) 
...
allMoments=1:101; % benchmark specification
...
noIncentLyingMoments = [1:93 95 97 99 101]; % doesn't include lying incentive
noINIMoments = [1:72 93:101]; % doesn't include I/NI moments
C_GOTVMoments=1:102; % include congressional GOTV
max_empirical=1:102;
...
paramLabels={'h0_v','h0_nv','r_v','r_nv','eta_v','eta_nv',...
     'mu_s_v','mu_s_nv','sigma_s_v','sigma_s_nv','S_svy_v','S_svy_nv',...
     'timeval_v','timeval_nv','mu_sv','mu_sn','sigma_sv','sigma_sn',...
     'rho','L','mu_eps','sigma_eps'};
      
noOfParams=22;
allParams = 1:noOfParams;
fixedL_Rho=[1:18 21:22]; 
fixedL_RhoSigEps=[1:18 21];
fixed_LRhoEps=[1:18];  
fixedRho=[1:18 20:22];
fixedRho_SigEps=[1:18 20:21];
fixedRho_MuSv=[1:14 16:18 20:22];
fixedRho_MuSn=[1:15 17:18 20:22];
fixedRho_MuSvSn=[1:14 17:18 20:22];
fixedRho_MuSvSn_SigSvSn=[1:14 20:22];
fixedRho_SigSvSn=[1:16 20:22];
...
Master_sigmas = [17]; % sigma_sv
Copy_sigmas = [18]; %sigma_sn
...
if L_setting == 'cons' 
svyEstimates  = [0.37, 0.37, 0.34, 0.34, 0.1, 0.1, -28, -28, 30, 30, ...
    1, 1, 42, 42, -4, -13, 15, 15, 0, cons_fixedL, fixed_mu_eps, fixed_sigma_eps]';
end
if L_setting == 'dist' 
svyEstimates  = [0.37, 0.37, 0.34, 0.34, 0.1, 0.1, -28, -28, 30, 30, ...
    1, 1, 42, 42, -4, -13, 15, 15, 0, 0.05, fixed_mu_eps, fixed_sigma_eps]';
end
...
paramsToUse=fixedRho;   % or fixedL_Rho
display 'fixedRho';
momentsToUse=allMoments;
display 'allMoments';

fixedParams=paramsToSim; 
...
%Draw random set for simulations. Note this is done only once and does not 
% vary in the estimation.
...
% NOTE: this picks many random start points for everything but the
% copy parameters

% Fix random seed
% get search inits also returns start space bounds for confirmation
rng(747)
[searchInits,lower_start,upper_start] = getSearchInits(noSearchInits,L_setting,sim_voters,rand_set,votesimFile);
for j=1:noOfParams
...
%Define the search space - depends on whether L is a constant or
%  distribution parameter
if L_setting == 'cons' 
lowerBound=[eps, eps, eps, eps, eps, eps, -100, -100, eps, eps, eps, eps, ...
    eps, eps, -100, -100, eps, eps, -1, eps, -500, eps];
upperBound=[1,     1,   1,   1, 0.5, 0.5,  100,  100, 100, 100, 100, 100, ...
    200, 200,  100,  100, 100, 100,  1, 50,  500, 500];
end
if L_setting == 'dist' 
lowerBound=[eps, eps, eps, eps, eps, eps, -100, -100, eps, eps, eps, eps, ...
    eps, eps, -100, -100, eps, eps, -1, eps, -500, eps];
upperBound=[1,     1,   1,   1, 0.5, 0.5,  100,  100, 100, 100, 100, 100, ...
    200, 200,  100,  100, 100, 100,  1, 1,  500, 500];
end
...
% Save temp files along the way (in case of crash)
disp('saving every 240 start points');

%first loop, groups of 240 start points
temp_k=0
if noSearchInits > 240 
    for temp_k = 240:240:(floor(noSearchInits/240)*240)

        parfor j=(temp_k-240+1):temp_k
            if mod(j,10)==0
                disp(j);
            end
           ...
           iter(j) = temp_iter;
           funcCount(j) = temp_funcCount;
        end
        save(strcat(outputDir,dataFile,'-prelim',num2str(temp_k),'.mat'));
    end
end

%any remaining start points (or all if < 240)
if temp_k < noSearchInits
    parfor j=(temp_k+1):noSearchInits
            if mod(j,10)==0
                disp(j);
            end
           [paramHats(:,j), fval(j), converged(j), sse(j), temp_iter, temp_funcCount] ...
        ...
        save(strcat(outputDir,dataFile,'-prelim',num2str(noSearchInits),'.mat'));
end
...
%transpose for easy transfer to excel
