% =================================================================
% Estimate the model
% Different V/NV parameters, assume a utility from talking about politics
% =================================================================
...
dataFile = strcat('utalk-',datestr(now,'dd-mmm-yyyy-HH-MM'));
outputFile = strcat(dataFile,'.txt');
diary(strcat(outputDir,outputFile));

% set votesim file that generates moments
votesimFile = @voteSimEndog_UTalk
calc_implicationsFile = @calc_implications_UTalk
...
%24 parameters including utility of talking about voting
noOfParams=24;
allParams = 1:noOfParams;
fixedRho=[1:18 20:24];
fixedRho_MuSv=[1:14 16:18 20:24];
fixedRho_UTalkV=[1:18 20:22 24];
...
% Note: fix mu sv or sn to 0 or fix utility of talking about politics 
% to 0 for either group
if L_setting == 'cons' 
svyEstimates  = [0.37, 0.37, 0.34, 0.34, 0.1, 0.1, -28, -28, 30, 30, ...
    1, 1, 42, 42, ...
    0, 0, 15, 15, 0, cons_fixedL, fixed_mu_eps, fixed_sigma_eps, ...
    0, 0]';
end
...
%Fixing Rho and Mu Sv
paramsToUse=fixedRho_MuSv; 
display 'fixedRho_MuSv';
...
rng(747)
[searchInits,lower_start,upper_start] = getSearchInits_UTalk(noSearchInits,L_setting,sim_voters,rand_set,votesimFile);
for j=1:noOfParams
...
% Note: range for utility of talking about politics -100 to 100
if L_setting == 'cons' 
lowerBound=[eps, eps, eps, eps, eps, eps, -100, -100, eps, eps, eps, eps, ...
    eps, eps, -100, -100, eps, eps, -1, eps, -500, eps, -100, -100];
upperBound=[1,     1,   1,   1, 0.5, 0.5,  100,  100, 100, 100, 100, 100, ...
    200, 200,  100,  100, 100, 100,  1, 50,  500, 500, 100, 100];
end
